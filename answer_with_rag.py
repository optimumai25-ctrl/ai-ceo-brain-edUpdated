...
def _public_enabled() -> bool:
    # dynamic read so UI can toggle it at runtime
    return os.getenv("PUBLIC_FACTS_ENABLED", "true").lower() in ("1", "true", "yes")
...
def resolve_date_window_from_query(q: str):
    ...
    if m:
        ...
        return (datetime(y, mo, d, 0, 0, 0), datetime(y, mo, d, 23, 59, 59))
...
def ask_gpt(
    ...
    if _use_client:
        resp = _client.chat.completions.create(model=COMPLETIONS_MODEL, messages=messages[-6:])
...
def answer(
    ...
    # If RAG found nothing, try public web search (if enabled)
    if not hits and _public_enabled():
        return web_answer(query, max_results=3)

    # If still nothing, ask GPT without sources
    if not hits:
        return ask_gpt(query, context="", chat_history=chat_history, structure="none")
...
if __name__ == "__main__":
    print(answer("Who is the president of USA?", k=5, restrict_to_meetings=True))
