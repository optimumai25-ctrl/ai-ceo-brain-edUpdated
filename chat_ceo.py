...
if mode == "🔁 Refresh Data":
    ...
    if "use_public_facts" not in st.session_state:
        st.session_state["use_public_facts"] = True  # default ON
    ...
    with colC:
        use_public_facts = st.checkbox(
            "🌐 Use public web for facts",
            value=st.session_state["use_public_facts"],
            key="use_public_facts",
        )
        # Pass setting to backend via env var (read at call time)
        os.environ["PUBLIC_FACTS_ENABLED"] = "true" if use_public_facts else "false"
